...
def calculate_tps(generate_func, method_name, max_new_tokens, use_cache, device=DEVICE, prompts=PROMPTS, reset_callback=None):
    '''
    ...
    for p in prompts:
        ...
        with torch.no_grad():
            output_ids = generate_func(input_ids, max_new_tokens=max_new_tokens, use_cache=use_cache)
...
if __name__ == '__main__':
    ...
    if main_model and draft_model:
        ...
        tps_main_without_cache = calculate_tps(generate_func=main_model.generate, max_new_tokens=max_new_tokens, method_name="main without cache", use_cache=False)
        tps_main_with_cache = calculate_tps(generate_func=main_model.generate, max_new_tokens=max_new_tokens, method_name="main with cache", use_cache=True, reset_callback=reset_main)

        # Calculate avg tps for draft model
        tps_draft_without_cache = calculate_tps(generate_func=draft_model.generate, max_new_tokens=max_new_tokens, method_name="draft without cache", use_cache=False)
        tps_draft_with_cache = calculate_tps(generate_func=draft_model.generate, max_new_tokens=max_new_tokens, method_name="draft with cache", use_cache=True, reset_callback=reset_draft)
        ...
        tps_speculative_without_cache = calculate_tps(generate_func=generate_func, max_new_tokens=max_new_tokens, method_name="speculative without cache", use_cache=False)
        tps_speculative_with_cache = calculate_tps(generate_func=generate_func, max_new_tokens=max_new_tokens, method_name="speculative with cache", use_cache=True, reset_callback=reset_both)
        
        print(f"\n>> Average tokens per second for main model, without cache: {tps_main_without_cache}, with_cache: {tps_main_with_cache}")
        print(f">> Average tokens per second for {draft_model_name} model, without cache: {tps_draft_without_cache}, with_cache: {tps_draft_with_cache}")
